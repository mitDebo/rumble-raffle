...
The CI/CD pipeline is built and verified end-to-end: a push to `main` runs tests, builds/pushes Docker images to GHCR, and deploys automatically to `raffle.kdubs.tech` — proven both on the initial deploy and on a follow-up code change. The backend now exposes three Kubernetes-style health endpoints (`/health`, `/ready`, `/startup`) via ASP.NET Core's health-check middleware, built ahead of need in case this ever runs on Kubernetes; `/ready` and `/startup` are tag-based so the database and (later) image storage checks fold in automatically as those land -- SignalR deliberately isn't one of them, since it's in-process middleware with no external dependency to probe, unlike a database or storage API. The frontend is currently a static placeholder page (no backend calls). Supabase is set up and EF Core is wired in: `/ready` now includes a real database check, and integration tests spin up a throwaway Postgres via Testcontainers rather than touching Supabase or relying on in-memory EF Core. Production credentials reach the droplet through a dedicated deploy step that writes them into a `.env` file Docker Compose reads, sourced from GitHub Actions secrets. The one pipeline piece still open is running EF Core migrations as their own gated CI step. Next: Phase 1 of the task breakdown (auth, SignalR).
